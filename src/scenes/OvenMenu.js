...
/**
 * @typedef {Object} OvenMeasurements
...
export default class OvenMenu extends Phaser.Scene {
    constructor() {
        super({ key: 'OvenMenu', active: false });
    ...
    initialiseOvenMenu(){
        ...
        this.createAnimation(measurements);
        this.createZones(measurements);

    }

    /**
     * Transfers the ingredient from the inventory to the oven if there are enough spaces on the oven.
    ...
    /**
     * Checks if a recipe exists with the current ingredients on the oven. If it exists, then get the meal name
    ...
    cook(){
        ...
        if (this.recipes.findRecipeByIngredients(ingredientNames, "Oven") != null){
            recipe = this.recipes.findRecipeByIngredients(ingredientNames, "Oven");
        ...
        this.restartOvenMenu();
    }

    /**
     * Initialises the background, select image, cancel button, and label for the oven menu.
     * @param {OvenMeasurements} measurements 
     */
    ...
    /**
     * Intialises the oven animation and its zone.
     * @param {OvenMeasurements} measurements 
     */
    createAnimation(measurements){
        if (!this.anims.exists('moving_oven')) { this.anims.createFromAseprite('oven'); }

        let pan = this.add.sprite(measurements.menuWidth/2 - 30, measurements.menuHeight/2 + 40, "bowl").setOrigin(0, 0).setScale(4);
        pan.play({ key: 'moving_oven', repeat: -1 });
