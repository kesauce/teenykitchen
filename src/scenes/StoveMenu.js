...
export default class StoveMenu extends Phaser.Scene {
    ...
    /**
     * Checks if a recipe exists with the current ingredients on the stove. If it exists, then get the meal name
    ...
    /**
     * Intialises the pan animation and its zone.
     * @param {StoveMeasurements} measurements 
     */
    createPan(measurements){
        if (!this.anims.exists('moving_pan')) { this.anims.createFromAseprite('pan'); }

        let pan = this.add.sprite(measurements.menuWidth/2 - 30, measurements.menuHeight/2, "pan").setOrigin(0, 0).setScale(4);
    ...
    /**
     * Initialises the stores each of the ingredient zones.
     * @param {StoveMeasurements} measurements 
     */
